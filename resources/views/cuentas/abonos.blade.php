...
<div class="page-header">
  ...
  </h3>  <form action="{{ route('cuentas.destroy', $cuenta->id) }}" method="POST" style="display: inline;" 
  ...
  <div class="row" style="margin-bottom: 5px;">
    <div class="col-md-6">
      ...
      <a class="btn btn-primary btn-sm" 
        href="{{ route('cuentas.abonos', [$cuenta->id, 'desde' => $desde, 'hasta' => $hasta ]) }}">
        Abonos
      </a>
      <a class="btn btn-default btn-sm" 
        href="{{ route('cuentas.gastos', [$cuenta->id, 'desde' => $desde, 'hasta' => $hasta ]) }}">
        Gastos
      </a>
    </div>
    <div class="col-md-6" align="right">
       <form action="{{ route('cuentas.gastos', $cuenta->id) }}" 
        method="GET" 
        class="form-inline">
    ...
    @if($abonos->count())
      <div class="table-responsive">
        <table class="table table-condensed">
          ...
          <tbody>
            @each('cuentas.abono', $abonos, 'abono')
          </tbody>
        </table>
        <div class="col-md-4" align="right">
          <p><b>Transferencia : </b>{{number_format($totalTransferencia,2)}}</p> 
          <p><b>Abono : </b>{{number_format($totalAbono,2)}}</p>
          <p><b>Gasto : </b>{{number_format($totalGasto,2)}}</p>
          <p><b>Balance :</b> {{number_format($balance,2)}} </p>
        </div>
        <div class="col-md-8" align="right">
          <div class="paginador">
            {{ $abonos->appends(Request::only('desde','hasta'))->links() }}
          </div>
        </div>
      @else
