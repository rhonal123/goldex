...
class CuentaPdf extends \TCPDF {
  ...
  public function Header() {
        ...
        $this->Cell(0,0, 'Listado de cuentas', 0, false, 'C', 0, '', 0, false, 'M', 'M');
        $this->Ln();
        if(!empty($this->desde)) {
	        $this->Ln();
					$this->SetXY(220,20);
    ...
    $header= true;
		foreach ($cuentas as $cuenta) {
			$transferencia = $cuenta->totalTransferencia($desde,$hasta);
			$abonos =  $cuenta->totalAbono($desde,$hasta);
			$gastos =  $cuenta->totalGasto($desde,$hasta);
      $balance = $abonos - $transferencia - $gastos;
			if($header){
				$this->Ln();
				$this->SetFont('times', null, 9);
				$this->SetFillColor(243, 255, 166);
		 		$this->Cell(90,7,"Cuenta", 1, 0, 'C', 1);
      ...
      $this->Cell(90,7,$cuenta->numero, 1, 0, 'C');
